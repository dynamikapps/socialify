...
class SocialMediaCaptionToolClaude:
            ...
            for platform in selected_platforms:
                if platform in prompts:  # Check if the platform was selected and has a defined prompt
                    ...
                    response = self.client.messages.create(
                        ...
                        temperature=0.7,
                        max_tokens=1024,
                        system=f"You are a social media management assistant providing creative and engaging captions for a blogger's social media posts. Ensure that each caption effectively highlights the essence of the blog post and is tailored to resonate with {self.target_audience} audience. Avoid adding extraneous characters before or after each caption. Always insert double spaces between sentences for easier reading.",
                        messages=[
                            {"role": "user", "content": prompt},
                        ],
                    )
                    captions[platform] = response.content[0].text.strip().strip(
                        '"').strip("'")
